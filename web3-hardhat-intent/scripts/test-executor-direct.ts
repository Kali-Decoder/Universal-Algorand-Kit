...
// Test user address (from Ethereum)
...
console.log("🧪 Testing Executor.execute_with_data() directly\n");
console.log(`Relayer: ${relayerAccount.addr}`);
console.log(`Executor App: ${executorAppId}`);
console.log(`TodoList App: ${todoAppId}`);
console.log(`User (padded): ${Buffer.from(userBytes).toString("hex")}\n`);

// Build the inner app args for TodoList.add_todo
...
console.log(`Relayer box: ${relayerBoxName.toString("base64")}`);
console.log(`User box: ${userBoxName.toString("base64")}\n`);
...
async function main() {
  try {
    ...
    console.log("✅ SUCCESS! TodoList intent executed via Executor");
  } catch (err: any) {
