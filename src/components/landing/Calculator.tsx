...
function RateBadge({ rates, isLoading, error }: { rates: Record<string, number> | null; isLoading: boolean; error: string | null }) {
  if (isLoading) return <span className="flex items-center gap-1 text-[0.6rem] text-[var(--color-muted)]"><span className="w-1.5 h-1.5 rounded-full bg-gray-400 inline-block" />Loading...</span>;
  ...
  return (
    <section id="calculator" className="py-24 md:py-32 px-5 md:px-8 lg:px-12 bg-black">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div>
            <p className="text-[var(--color-muted-2)] text-[0.7rem] font-semibold uppercase tracking-[0.2em] mb-4">Intent simulator</p>
            <h2 className="section-headline text-white">
              Simulate cross-chain<br />execution.
            </h2>
          </div>
          <p className="text-[var(--color-muted)] text-[0.9rem] leading-relaxed max-w-xs">
        ...
        <motion.div
          ...
          style={{
            background: 'var(--black-1280)',
            border: '1px solid var(--white-10)',
            backdropFilter: 'blur(24px)',
            boxShadow: '0 0 0 1px var(--white-04) inset, 0 32px 80px var(--black-60)',
          }}
        >
          <div
            className="absolute top-0 left-0 right-0 h-px"
            style={{ background: 'linear-gradient(90deg, transparent 10%, var(--accent-35) 50%, transparent 90%)' }}
          />
          <div
            className="absolute -top-32 left-1/2 -translate-x-1/2 w-64 h-64 rounded-full blur-3xl pointer-events-none"
            style={{ background: 'radial-gradient(circle, var(--accent-07) 0%, transparent 70%)' }}
          />
          ...
          <div className="relative grid md:grid-cols-[1fr_auto_1fr] gap-5 items-stretch mb-5">
            ...
            >
              <p className="text-[var(--color-muted-2)] text-[0.7rem] font-semibold uppercase tracking-[0.15em] mb-4">Intent Payload</p>
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-[var(--color-muted-2)] text-lg">$</span>
                <input
                  type="number"
                  value={amount}
                  ...
                  min="0"
                />
                <span className="text-[var(--color-muted-2)] text-sm font-medium">Units</span>
              </div>
              <div className="relative">
                <button
                  ...
                  style={{ background: 'var(--white-04)', border: '1px solid var(--white-08)' }}
                >
                  <span className="font-mono text-[var(--color-accent)] text-xs">{selectedChain.icon}</span>
                ...
                {showChainDrop && (
                  <div
                    ...
                    style={{
                      background: 'var(--black-1495)',
                      border: '1px solid var(--white-10)',
                      backdropFilter: 'blur(16px)',
                      boxShadow: '0 20px 60px var(--black-70)',
                    }}
                  >
                    {chains.map((c) => (
                      <button
            ...
            <div className="flex flex-row md:flex-col items-center justify-center gap-2 py-2">
              <motion.div
                ...
                style={{ background: 'var(--accent-08)', border: '1px solid var(--accent-20)' }}
              >
                <ArrowLeftRight size={16} className="text-[var(--color-accent)]" />
              </motion.div>
              <span className="text-[var(--color-muted-2)] text-[0.65rem] font-medium whitespace-nowrap">via Algorand</span>
            </div>
            ...
            >
              <p className="text-[var(--color-muted-2)] text-[0.7rem] font-semibold uppercase tracking-[0.15em] mb-4">Execution Output</p>
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-[var(--color-muted-2)] text-lg">{selectedCorridor.symbol}</span>
                <span className="flex-1 text-[var(--color-accent)] text-[2.5rem] md:text-[3rem] font-black font-mono leading-none tracking-tight">
                  <AnimatedNumber value={receivedNum > 0 ? receivedNum : 0} decimals={0} />
                </span>
                <span className="text-[var(--color-muted-2)] text-sm font-medium">{selectedCorridor.currency}</span>
              </div>
              <div className="relative">
                <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
            {[
              ...
              >
                <div className="flex items-center gap-1.5 text-[var(--color-muted-2)] text-[0.65rem] mb-1.5">
                  <Clock size={10} />
                  {item.label}
                </div>
                ...
                <div className="text-[var(--color-muted-3)] text-[0.62rem] mt-0.5">{item.sub}</div>
              </div>
            ))}
            ...
            <div
              className="rounded-xl p-3"
              style={{ background: 'var(--black-40)', border: '1px solid var(--white-06)' }}
            >
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-1.5 text-[var(--color-muted-2)] text-[0.65rem]">
                  <Clock size={10} />
                  Route rate
                </div>
              ...
              <div className="text-[var(--color-muted-3)] text-[0.62rem] mt-0.5">
            ...
            <div
              className="rounded-xl p-3"
              style={{ background: 'var(--black-40)', border: '1px solid var(--accent-15)' }}
            >
              <div className="flex items-center gap-1.5 text-[var(--color-muted-2)] text-[0.65rem] mb-1.5">
                <Zap size={10} className="text-[var(--color-accent)]" />
                Execution time
              </div>
              <div className="font-mono font-bold text-sm text-[var(--color-accent)]">10–30 seconds</div>
              <div className="text-[var(--color-muted-3)] text-[0.62rem] mt-0.5">end-to-end</div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 hover:bg-white/[0.07] border border-white/[0.08] text-[var(--color-muted)] hover:text-white text-sm font-medium px-5 py-3 rounded-full transition-colors"
              style={{ background: 'var(--white-03)' }}
            >
              {showSavings ? 'Hide comparison' : 'Compare to bridges'}
            </button>
            <a
              href="/docs"
              className="flex-1 flex items-center justify-center gap-2 bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-black font-bold text-sm py-3 rounded-full transition-all duration-200"
            >
            ...
            {showSavings && savings > 0 && (
              ...
              >
                ...
                >
                  ...
                  <div>
                    <div className="text-[var(--color-accent)] font-bold text-sm mb-0.5">
